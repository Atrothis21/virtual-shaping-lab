      ...
      phases: [
        {
          ...
          stimuli: { cs_plus: params.cs_plus, cs_minus: params.cs_minus },
...
function App() {
  const [params, setParams] = React.useState({
    n_trials: 140, alpha: 0.2, attention_plus: 0.35, attention_minus: 0.35, cs_plus: ["tone"], cs_minus: ["noise"],
  });
  ...
  return (
    <>
      <h1>Learned Irrelevance (Attention) Preset</h1>
      <p>Differential acquisition with low predictiveness and reduced attention.</p>
      <div className="actions"><button className="btn" onClick={() => { window.location.href = "/ui/presets.html"; }}>Back to Presets</button></div>
      <div className="panel"><h3>What This Preset Shows</h3><p>Weak cue predictiveness can drive attention downward and blunt learning.</p></div>
      ...
      <div className="panel">
        ...
        <select multiple value={params.cs_plus} onChange={onCSPlus}>{STIMULI.map((s) => <option key={s} value={s} disabled={params.cs_minus.includes(s)}>{s}</option>)}</select>
        <br /><br />
        <label>CS- Stimuli</label>
        <select multiple value={params.cs_minus} onChange={onCSMinus}>{STIMULI.map((s) => <option key={s} value={s} disabled={params.cs_plus.includes(s)}>{s}</option>)}</select>
      </div>
