...
function buildPayload(params) {
  ...
  const payload = {
    experiment: {
      ...
      phases: [
        { name: "Rapid Reacquisition: Acquisition", protocol: "acquisition", stimuli: { cs_plus: params.cs_plus }, params: { n_trials: params.acq_n_trials, alpha: params.alpha, gamma: params.gamma } },
...
function validate(params) {
  if (params.acq_n_trials < 1 || params.ext_n_trials < 1 || params.reacq_n_trials < 1) throw new Error("Trial counts must be at least 1");
...
function App() {
  const [params, setParams] = React.useState({ acq_n_trials: 80, ext_n_trials: 70, reacq_n_trials: 80, alpha: 0.2, gamma: 0.0, attention: 0.85, cs_plus: ["tone"] });
  ...
  return (
    <>
      <h1>Rapid Reacquisition (Attention) Preset</h1>
      <p>Acquisition, extinction, then reacquisition on the same cue.</p>
      <div className="actions"><button className="btn" onClick={() => { window.location.href = "/ui/presets.html"; }}>Back to Presets</button></div>
      <div className="panel"><h3>What This Preset Shows</h3><p>Attention can remain elevated after extinction and support faster relearning.</p></div>
