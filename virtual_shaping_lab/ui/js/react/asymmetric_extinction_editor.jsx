...
function buildPayload(params) {
  ...
  return {
    experiment: {
      ...
      phases: [
        {
          name: "Asymmetric Extinction: Compound Acquisition",
          protocol: "compound_acquisition",
          stimuli: { compound },
          params: {
            n_trials: params.n_trials_acq,
        ...
        {
          name: "Asymmetric Extinction: Nonreinforcement",
          protocol: "nonreinforcement",
          stimuli: { cs_plus: [params.stim_1, params.stim_2] },
          params: {
            n_trials: params.n_trials_ext,
...
function validate(params) {
  if (params.n_trials_acq < 1) throw new Error("Acquisition trials must be at least 1");
  if (params.n_trials_ext < 1) throw new Error("Extinction trials must be at least 1");
  ...
  if (params.alpha_ext < 0 || params.alpha_ext > 1) throw new Error("alpha_ext must be 0-1");
...
function AsymmetricExtinctionApp() {
  ...
  return (
    <>
      <h1>Asymmetric Extinction Preset</h1>
      <p>Compound acquisition followed by nonreinforcement with unequal cue salience.</p>
      ...
      <div className="panel">
        <h3>Trials</h3>
        <label>Acquisition Trials: <span>{params.n_trials_acq}</span></label>
        <input type="range" min="1" max="500" value={params.n_trials_acq} onChange={(e) => setParams((p) => ({ ...p, n_trials_acq: +e.target.value }))} />
        <label>Extinction Trials: <span>{params.n_trials_ext}</span></label>
        <input type="range" min="1" max="500" value={params.n_trials_ext} onChange={(e) => setParams((p) => ({ ...p, n_trials_ext: +e.target.value }))} />
      </div>
      ...
      <div className="panel">
        ...
        <label>Alpha Extinction: <span>{params.alpha_ext}</span></label>
        <input type="range" min="0" max="1" step="0.05" value={params.alpha_ext} onChange={(e) => setParams((p) => ({ ...p, alpha_ext: +e.target.value }))} />
      </div>
...
root.render(<AsymmetricExtinctionApp />);
