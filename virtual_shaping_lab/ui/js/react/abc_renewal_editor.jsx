  ...
  const payload = {
    experiment: {
      ...
      params: {
        ...
        context_c: params.context_c,
      },
      salience: salienceMap,
      attention: attentionMap,
    },
    report: { preset: "abc_renewal" },
  };
...
function validate(params) {
  ...
  if (!params.context_c) throw new Error("Context C is required");
}

function ABCRenewalApp() {
  ...
  return (
    <>
      <h1>ABC Renewal Preset</h1>
      <p>Protocol: Acquisition in A -&gt; Extinction in B -&gt; Probe in C.</p>
      ...
      <div className="panel">
        ...
        <select value={params.context_c} onChange={(e) => setParams((p) => ({ ...p, context_c: e.target.value }))}>
          {CONTEXTS.map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
      </div>
