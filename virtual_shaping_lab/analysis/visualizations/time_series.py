...
class LinePlot(Visualization):
    """
    Generic line plot for time-series metrics.
    ...
    def _phase_spans(self, records):
        ...
        name_key = "subphase_name" if "subphase_name" in records[0] else "phase"
    ...
    def render(self, records, metrics=None) -> None:
        if not records:
            raise ValueError("LinePlot received no records")

        # Prefer records; fallback to metrics
        ...
        if not series and metrics:
            ...
            if not series:
                raise ValueError(f"LinePlot received empty series for metric '{metric_name}'")
        elif not series:
            raise ValueError("LinePlot could not infer series from records")

        self.fig, ax = plt.subplots()
        ax.plot(series)
        ax.set_xlabel(self.xlabel)
        ax.set_ylabel(self.ylabel)
        ax.set_title("Prediction Time Series", pad=18)
        ax.grid(True)
    ...
    def save(self, path) -> None:
        if self.fig is None:
            raise RuntimeError(
                "LinePlot.render() must be called before save()"
            )
